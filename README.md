...
$ python setup.py install --user # 默认安装到当前用户的~/.local/lib/pythonx.x/site-packages
...
access_token_has_expired  # token 过期(有效期一周)，需要重新申请(清空配置文件中TOKEN字段，运行程序手工获取新的TOKEN) 
rate_limit_exceeded1  # 请求频率和数量过高
rate_limit_exceeded2  # IP 访问速度限制
KeyError      # IP 访问速度限制，被封禁，导致无法获取 JSON 数据
ValueError: No JSON object could be decoded # 访问速度限制，IP被封禁， 导致无法获取 JSON 数据
...
### Changelog
```
__v0.0.7 [2014-04-22]__
* 优化配置文件读写
* 优化 token 获取，增加自动刷新 token 机制
__v0.0.6 [2014-04-14]__
* 优化安装，满足非 root 用户安装和使用
* 完善 README
* 修复错误
