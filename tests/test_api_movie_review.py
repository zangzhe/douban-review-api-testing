...
class TestApiMovieReview(DoubanClientTestBase):
    ...
    def test_new_update_delete_review_function_v2(self):
        # new
        ret = self.client_v2.movie.review.new(self.movie_id, \
        ...
        ret = self.client_v2.movie.review.update(review_id, \
            self.title, content)
    ...
    def test_new_review_exception_movie_id_v2(self):
        ...
        ret = self.client_v2.movie.review.new(invalid_movie_id, \
            self.title, self.content)
        self.assertTrue(isinstance(ret, dict))
    ...
    def test_new_review_exception_rating_v2(self):
        ...
        ret = self.client_v2.movie.review.new(self.movie_id, self.title, \
    ...
    def test_update_review_exception_review_id_v2(self):  
        ...
        content = self.content * 2
        ret = self.client_v2.movie.review.update(invalid_review_id, \
            self.title, content)
        self.assertTrue(isinstance(ret, dict))
    ...
    def test_update_review_exception_title_v2(self):
        ...
        review_id = ret['id']
        invalid_title = self.title*200 # too long title
        ret = self.client_v2.movie.review.update(review_id, invalid_title,\
         self.content)    
        self.assertTrue(isinstance(ret, dict))
        self.assertTrue(invalid_title.startswith(ret['title']))
        review_id = ret['id']
    ...
    def test_update_review_exception_content_v2(self):
        ...
        review_id = ret['id']
        invalid_content = "TooShort" # too short content
        ret = self.client_v2.movie.review.update(review_id, self.title,\
         invalid_content)    
        try:
            self.assertTrue(isinstance(ret, dict))
